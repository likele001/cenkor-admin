...
<template>
  <div ref="rootRef" class="relative">
    <button
      type="button"
      class="px-2.5 py-1.5 rounded-md text-sm text-slate-600 hover:bg-slate-100 inline-flex items-center gap-1.5"
    ...
    >
      <div
        v-if="open"
        class="absolute right-0 mt-1.5 w-40 rounded-lg border bg-white shadow-lg py-1 z-50"
        role="menu"
      >
        <button
          ...
          :class="{ 'text-indigo-600 bg-indigo-50/50': localeStore.locale === o.code }"
          role="menuitem"
          @click="pick(o.code)"
        >
          <span>{{ o.flag }}</span>
